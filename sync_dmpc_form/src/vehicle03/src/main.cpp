...
void Initialize(ros::NodeHandle& n){
	...
	#if 0
	// test setting 2
	xinit=5.0;yinit=-5.0;thetainit=3.14/4.0*3.0;
	...
	std::vector<std::vector<double>> neig2(N+1,std::vector<double>{-5.0,-1.0});
	#endif

	#if 0
	// test setting for formation 1
	xinit=2.0;yinit=-6.0;thetainit=1.57;
	...
	xinit=3.0;yinit=-7.0;thetainit=3.14/4.0;
	xr=1.0;yr=4.0;thetar=0.0;
	std::vector<double> obst1 = {0.0,0.0};
	std::vector<std::vector<double>> neig1(N+1,std::vector<double>{-3.0,-7.0});
	std::vector<std::vector<double>> neig2(N+1,std::vector<double>{0.0,-7.0});
	#endif
	...
	neig_pub= n.advertise<mymsg::neighborpos>("neig3pos", 10);
        ...
        sub2 = n.subscribe("neig2pos", 1000, NeighborCallback2);
